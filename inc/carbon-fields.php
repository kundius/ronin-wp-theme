...
function register_carbon_fields_blocks()
{
  ...
  Container::make('post_meta', 'Главная')
    ...
    ->add_tab('Начальный экран', [
      ...
      Field::make('image', 'intro_bg_video', 'Фоновое видео'),
  ...
  Block::make('partials_about_info', 'Блок "Информация и запись"')
    ->add_fields([
      Field::make('separator', 'separator', 'Блок "Информация и запись"'),
    ])
    ->set_category('layout')
    ->set_mode('edit')
    ->set_icon('shortcode')
    ->set_render_callback(function ($fields, $attributes, $inner_blocks) {
      get_template_part('partials/about-info', null, [
        'fields' => $fields,
        'attributes' => $attributes,
        'inner_blocks' => $inner_blocks,
      ]);
